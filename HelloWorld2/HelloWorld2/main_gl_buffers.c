...
void	create_index_buffer(t_scop *scop)
{
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * scop->indices_count, scop->indices, GL_STATIC_DRAW);
}
...
void	create_vertices_buffer(t_scop *scop)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * scop->vert_num, scop->verts, GL_STATIC_DRAW);
...
void	create_colors_buffer(t_scop *scop)
{
	...
	glBufferData(GL_ARRAY_BUFFER, (sizeof(float) * (scop->vert_num + (scop->vert_num / 3))), scop->colors, GL_STATIC_DRAW);
